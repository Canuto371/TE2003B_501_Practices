...
void servir_jugo(uint8_t tanque_idx) {
    ...
    uint8_t bomba_idx = tanques_config[tanque_idx].bomba_idx;

    HAL_LCD_Clear();
    HAL_LCD_Print("Sirviendo ");
    HAL_LCD_Print(nombre);

    // Activar bomba específica para este tanque
    HAL_Bomba_Activar(bomba_idx, true);
    delayMs(TIEMPO_SERVIR_MS);
    HAL_Bomba_Activar(bomba_idx, false);
