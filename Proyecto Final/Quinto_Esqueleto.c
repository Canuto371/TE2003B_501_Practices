//Quinto esqueleto Proyecto Final FruitXpress
...
// Pump GPIO pin
#define BOMB_GPIO GPIOA
#define BOMB_GPIO_PORT PORTA
...
static uint32_t pumpStartTime = 0; // in microseconds
...
int main(void){
	...
	PORTA->PCR[BOMB_PIN] = PORT_PCR_MUX(1); // Mux as GPIO
	BOMB_GPIO->PDDR |= (1U << BOMB_PIN);
	BOMB_GPIO->PCOR |= (1U << BOMB_PIN);
...
void show_selection_menu() {
    ...
    char key = 0;
    while (!key) {
        key = convert_key(keypad_getkey());
    }

    uint8_t sensorIndex = 255; // valor inválido por defecto
    const char* sabor = "";
    ...
    // Verificar nivel
    uint32_t distancia = medir_distancia_cm(sensorIndex);
    LCD_command(0x01);
    ...
    if (distancia < UMBRAL_CM) {
        ...
        LCD_command(0xC0);
        LCD_print("Sirviendo...");
        activar_bomba_por_3s();
        LCD_command(0x01);
        LCD_print("Listo!");
    } else {
        LCD_print("Nivel bajo:");
        LCD_command(0xC0);
        LCD_print(sabor);
    }
...
//Manejo de bombas
void activar_bomba_por_3s(void) {
    BOMB_GPIO->PSOR |= (1U << BOMB_PIN); // Enciende bomba
    delayMs(3000);                        // Espera 3 segundos
    BOMB_GPIO->PCOR |= (1U << BOMB_PIN); // Apaga bomba
}